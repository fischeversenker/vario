...
CompareableType constrain(CompareableType v, CompareableType hi, CompareableType lo){
    return (v < lo) ? lo : (hi < v) ? hi : v;
    // C++17 -> return std::clamp<CompareableType>(x, a, b); 
}

template <class MappedType>
constexpr MappedType map(MappedType value, MappedType fromLow, MappedType fromHigh, MappedType toLow, MappedType toHigh)
{
