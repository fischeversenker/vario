...
void Vario::update() {
  ...
  if (now - _lastUpdate >= VARIO_UPDATE_FREQUENCY) {
    ...
    _verticalSpeed =
        (currentAltitude - _lastAltitude) * (1000 / VARIO_UPDATE_FREQUENCY);
    _lastAltitude = currentAltitude;
    _lastUpdate = now;
  }
}

float Vario::getVerticalSpeed() { return _verticalSpeed; }

float Vario::getAltitude() { return _bmp.readAltitude(); }
